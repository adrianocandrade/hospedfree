...
return new class extends Migration {
    private const ITEM_ID = 'hf-admin-premium-subdomains';
    private const ITEM_ACTION = '/admin/hosting/premium-subdomains';
    ...
    public function up(): void
    {
        $this->updateMenus(function (array $items): array {
            ...
            $plans = collect($items)->search(
                fn($candidate) => is_array($candidate) &&
                    ($candidate['id'] ?? null) === 'hf-admin-hosting-plans',
            );
            array_splice(
                $items,
                $plans === false ? count($items) : $plans + 1,
                0,
                [$item],
            );
