...
const columnConfig: ColumnConfig<NormalizedModel>[] = [
  {
    key: 'dragHandle',
    width: 'w-10.5 flex-shrink-0',
    header: () => <Trans message="Drag handle" />,
    hideHeader: true,
    body: () => (
      <DragHandleIcon className="hover:text cursor-pointer text-muted-foreground" />
    ),
  },
  {
    key: 'name',
...
>(({item}, ref) => {
  return (
    <DragPreview ref={ref}>
      {() => (
        <div className="rounded-sm bg-secondary p-2 text-base shadow-sm">
          {item.name}
        </div>
      )}
    </DragPreview>
