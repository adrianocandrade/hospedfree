...
export const ChannelsDatatableColumns: ColumnConfig<Channel>[] = [
  ...
  },
  {
    ...
    body: channel => (
      <Link to={`${channel.id}/edit`} className="text-muted-foreground">
        <IconButton size="md">
          <EditIcon />
        </IconButton>
      </Link>
...
function InternalColumn({channel}: ChannelNameProps) {
  ...
  const internalLabel = channel.internal ? (
    ...
    >
      <div>
        <Chip className="w-max" size="xs" radius="rounded-card">
  ...
  return (
    <div className="flex items-center gap-1.5">
      {internalLabel}
      {isHomepage ? (
        <HomeIcon className="text-muted-foreground" size="sm" />
      ) : null}
    </div>
  );
}
