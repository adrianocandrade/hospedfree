import { UseInfiniteQueryResult, UseSuspenseInfiniteQueryResult } from '@tanstack/react-query';
...
export function InfiniteScrollSentinel({
    ...
    content = !isInitialLoading && hasNextPage && (
      <div className={clsx('flex items-center gap-2', loaderMarginTop)}>
        ...
        <Button
          size={size === 'md' ? 'sm' : 'xs'}
          className={clsx(
            size === 'sm' ? 'min-h-6 min-w-24' : 'min-h-9 min-w-28',
          )}
          variant="outline"
          color="primary"
          onClick={() => {
  ...
  return (
    <div
      style={style}
      className={clsx('w-full', className, hasNextPage && 'min-h-9')}
      role="presentation"
    >
