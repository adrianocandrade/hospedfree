...
export function CommentDatatableItem({
  ...
  return (
    <div
      className={clsx('border-b p-3.5', comment.deleted && 'bg-destructive/6')}
    >
      {comment.commentable && (
        <CommentableHeader
      ...
      <div className="flex items-start gap-2.5 pt-3.5 md:pl-5">
        {comment.user ? (
        ...
        <div className="flex-auto">
          ...
          {isEditing ? (
            ...
            <Fragment>
              <div className="my-3.5 text-sm">{comment.content}</div>
...
function CommentableHeader({
  ...
  return (
    <div className="flex items-center">
      ...
      {commentable.image && (
        <img
          className="mr-1.5 h-5 w-5 overflow-hidden rounded-sm object-cover"
          src={commentable.image}
          alt=""
        />
      )}
      <div className="mr-1 text-sm">{commentable.name}</div>
      <div className="text-xs text-muted-foreground">({commentable.model_type})</div>
    </div>
  );
}
...
function UserDisplayName({user, show}: UserDisplayNameProps) {
  ...
  return <div className="text-muted-foreground">{user[show]}</div>;
}
