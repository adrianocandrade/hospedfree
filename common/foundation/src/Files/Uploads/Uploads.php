...
class Uploads
{
    ...
    public static function getAllTypes(): array
    {
        ...
        $localBackend = Arr::first(
            settings('uploading.backends') ?? [],
            fn($backend) => ($backend['type'] ?? null) === 'local',
        );
        $localBackendId = $localBackend['id'] ?? null;
        $types = [];
        ...
        foreach ($typeConfig as $name => $config) {
            ...
            // New upload types can be added after the persisted uploading
            // setting was created. Keep them usable until that setting is
            // migrated by assigning the configured local backend.
            if (empty($mergedConfig['backends']) && $localBackendId) {
                $mergedConfig['backends'] = [$localBackendId];
            }

            $types[] = new UploadType($name, $mergedConfig);
        }
