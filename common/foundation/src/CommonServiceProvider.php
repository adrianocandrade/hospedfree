...
use Common\Core\Middleware\SecurityHeaders;
...
use Common\Settings\Mail\EmbedBrandingLogoInOutgoingEmail;
...
class CommonServiceProvider extends ServiceProvider
{
    ...
    private function loadConfig(): void
    {
        ...
        // Laravel's default services config reads RESEND_KEY, while the
        // settings manager historically stores this credential as
        // RESEND_API_KEY. The framework config is loaded first and would
        // otherwise replace the configured MeuLinkBio value with null.
        $resendApiKey = OutgoingMailConfiguration::resolveResendApiKey(
            env('RESEND_API_KEY'),
            config('services.resend.key'),
        );
        if ($resendApiKey !== null) {
            config()->set('services.resend.key', $resendApiKey);
        }
    ...
    private function registerEventListeners(): void
    {
        Event::listen(
            ...
            EmbedBrandingLogoInOutgoingEmail::class,
        );
